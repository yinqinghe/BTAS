                    ...
                    acc.push({
                        ...
                        Location:
                            azure?.location?.countryOrRegion && azure?.location?.state && azure?.location?.city
                                ...
                                : undefined,
                        ...properties,
                        DeviceName: azure.deviceDetail.displayName || undefined,
                        OS_browser: `${azure.deviceDetail.operatingSystem}\\${azure.deviceDetail.browser}` || undefined,
                    ...
                    if (logObj['integration'] == 'Wazuh-MDE') {
                        ...
                        if (evidence) {
                            ...
                            loggedOnUsers.forEach(function (loggedOnUser) {
                                if (loggedOnUser) {
                                    if (!devices['loggedOnUsers']) {
                                        devices['loggedOnUsers'] = [];
                                    }
                                    devices['loggedOnUsers'].push({
                                        loggedOnUser: `${loggedOnUser['accountName']}\\\\${loggedOnUser['domainName']}(DomainName)`
                                    });
                                }
                                console.log('===', devices);
                            });
                            for (const evidenceItem of evidence) {
                                if (evidenceItem.entityType == 'Registry' && evidenceItem.registryKey != undefined) {
                                    if (evidenceItem.registryHive != undefined) {
                                        ...
                                        evidences['registry'].push({
                                            registryKey: `${evidenceItem.registryKey}`,
                                            registryValue: evidenceItem.registryValue
                                        });
                                    }
                                }
                                ...
                                if (evidenceItem.entityType === 'Process') {
                                    ...
                                    if (
                                        ...
                                        processEntry['Decode_Cmd'] = atob(
                                            evidenceItem.processCommandLine
                                                .split(' ')
                                                [cmd_length - 1].replace(/['"]/g, '')
                                        );
                                    }
                                    processEntry['cmd'] = processCommandLine;
    ...
    function parseLog(rawLog) {
        const alertInfo = rawLog.reduce((acc, log) => {
            try {
                ...
                if (DecoderName == 'playtechsecurityevents') {
                    let PTSE = JSON.parse(log)['PlayTechSecurityEvents'];
                    result = {
                        createtime: PTSE.TimeGenerated,
                        EventSourceName: PTSE.EventSourceName,
                        Computer: PTSE.Computer,
                        ...
                        TargetUserSid: PTSE.TargetUserSid,
                        WorkstationName: PTSE.WorkstationName,
                        IpAddress: PTSE.IpAddress
                    };
                    acc.push(result);
                    console.log('===', acc);
                }
